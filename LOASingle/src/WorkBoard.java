...
public class WorkBoard extends Board {
    ...
    int h_value() {  
    	...
    	for(int zero_sum = 0; zero_sum < 2; zero_sum++){
        	//Weighting Values
        	double w_quad = -3;		//Weighting for Quads
        	double w_uni  = 1;		//Weighting for Uniformity
        	double w_cent = 4;		//Weighting for Centralization
...
int max_value(int alpha, int beta, int depth){
	   	if(connected(to_move) || connected(opponent(to_move)) || depth == 0){
	   		numLeafNodes++;
